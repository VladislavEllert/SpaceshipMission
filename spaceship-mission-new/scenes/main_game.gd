...
func close_pipe_game() -> void:
	...
	room_index = 3
	_load_room(room_index)
	if pipe_game_solved and current_room:
		var banner := current_room.get_node_or_null("SolvedBannerPipeGame")
		if banner:
			banner.visible = true
...
func close_jumper() -> void:
	...
	room_index = 3
	_load_room(room_index)
	if jumper_solved and current_room:
		var banner := current_room.get_node_or_null("SolvedBannerJumper")
		if banner:
			banner.visible = true
